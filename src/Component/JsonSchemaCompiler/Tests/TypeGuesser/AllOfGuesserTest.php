...
class AllOfGuesserTest extends AbstractGuesserTester
{
    ...
    public static function providerData(): \Generator
    {
        ...
        yield [new JsonSchema(allOf: [new JsonSchema(name: 'Test', enum: [5, 10, 12, 15])]), new EnumType('TestEnum', [5, 10, 12, 15], CompiledType::INTEGER)];
        yield [new JsonSchema(allOf: [new JsonSchema(type: Type::INTEGER), new JsonSchema(name: 'Test', enum: [5, 10, 12, 15]), new JsonSchema(minimum: 5, maximum: 15)]), new EnumType('Test1Enum', [5, 10, 12, 15], CompiledType::INTEGER)];
    }
}
